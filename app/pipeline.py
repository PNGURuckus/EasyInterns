...
async def scrape_and_store(config):
    ...
    print(f"[scrape] companies(L={len(config.get('lever_companies', []))}, GH={len(config.get('greenhouse_companies', []))}) "
...
def get_ranked(profile: CandidateProfile, limit=200):
    rows = fetch_all(limit=limit*2)
    opps = [Opportunity(
        source=r.source, company=r.company, title=r.title,
        location=r.location, apply_url=r.apply_url, description_snippet=r.description_snippet,
        posted_at=r.posted_at, remote_friendly=r.remote_friendly, job_id=r.job_id,
